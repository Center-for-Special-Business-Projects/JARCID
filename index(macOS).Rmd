...
# Please take note that this script will take a few hours to complete. It needs further optimization but for now, the most time consuming tasks are generating a GeoJSON with Ottawa and Gatineau together and creating the maps from the building polygons. It can take about 5 hours to complete depending on the environment. When processing one city, it is much faster but then it depends on its size and the number of buildings it has to process. Running it in a cloud would probably be faster.
...
jpeg(MapDate, width=1920, height=1166)
  gg <- ggplot() + 
    geom_polygon(data = BuildOttGat, aes(x = long, y = lat, group = group), fill=NA, color="white") + 
    geom_polygon(data = BuildOttGat, aes(x = long, y = lat, group = group), fill=NA, color="white")
    theme1 +
