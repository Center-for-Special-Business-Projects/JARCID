...
# Please take note that this script can take about 2 hours to complete depending on the environment (computer, OS) and the size of the cities and the number of buildings.
...
# Import geometries. You need a poly file for the city you are monitoring. See documentation for more info. Recommendation is to use a standard geography like census subdivision or CMA (Statistics Canada). The use of the poly format is for osmosis, the library we use to do the extraction from OSM. The code could be adapted for OSMIUM, another library which uses GeoJSON instead making this step more convenient.

system(paste("cp ~/Projects/OSMdata/Geometries/*.poly", datePath))

# OSM data download and extraction ---------------------------

# The first part requires system commands line. This was done on a mac. On Windows, system() 
# must be replaced by shell(). These commands will run in terminal or a shell and use Osmosis to do two things:

# a) subset osm data to the CSD of a municipality (Ottawa and Gatineau)
# b) subset the buildings (ways), then convert the resulting osm files for both cities into a GeoJSON.

# 1. Ottawa

# Here, "\" needs to be escaped to work (with another \) (on macOS)   
...
# Estimates and updating the dataset ---------------------------
...
# Create a name with timestamp for the map
...
jpeg(MapDate, width=1920, height=1166)
  gg <- ggplot() + 
    ...
    theme1 +
    annotate("text", label = date, x = -76.2, y = 45.05, size = 14, colour = "white")
print(gg)
dev.off()
...
#Some calculations and estimates will still be done below for each segments.

#Notes: all charts are created with ggplot2. The use of plotly (ggplotly) is only there to benefit from the autoresizing of ggplotly. This ensures that the plots are always the right size for the space they are given. The code will also make use of the captured date of the extraction where necessary.
